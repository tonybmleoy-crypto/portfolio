              ...
              <div className="relative aspect-[556/461] w-full overflow-hidden rounded-[28px] bg-black/10">
                <Image
                  ...
                  className={`object-cover transition-transform duration-500 ${
                    project.available ? "group-hover:scale-105" : "opacity-70 grayscale"
                  }`}
                />
                {!project.available && (
                  <span className="absolute right-3 top-3 rounded-full bg-surface/90 px-3 py-1 text-xs text-muted">
                    {content.comingSoon}
                  </span>
                )}
              </div>
              <div className="flex flex-1 flex-col gap-4 px-3 pb-3">
                <div>
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    <span className="text-2xl font-medium tracking-tight">{project.tag}</span>
                  </div>
                    ...
                    <div className="flex items-center gap-6">
                      {project.timeline && (
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="text-lg font-medium tracking-tight">{project.timeline}</span>
                        </div>
                      )}
                      {project.users && (
                        <div className="flex flex-col gap-1">
                          <span className="text-sm text-foreground/50">{content.usersLabel}</span>
                          <span className="text-lg font-medium tracking-tight">{project.users}</span>
                        </div>
                      )}
                    </div>
